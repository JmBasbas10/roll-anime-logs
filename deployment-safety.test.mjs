...
test("secret environment variables remain server-only", async () => {
  ...
  assert.match(example, /ROBLOX_OAUTH_CLIENT_SECRET=/);
  assert.match(example, /AUTH_SESSION_SECRET=/);
  assert.match(example, /ADMIN_USERNAME=/);
  assert.match(example, /ADMIN_PASSWORD=/);
  assert.doesNotMatch(example, /NEXT_PUBLIC_SUPABASE_SECRET_KEY/);
  assert.doesNotMatch(example, /NEXT_PUBLIC_ROBLOX_OAUTH_CLIENT_SECRET/);
});

test("admin access and player edits are guarded", async () => {
  const [auth, middleware, playerRoute, migration] = await Promise.all([
    read("../lib/auth.ts"),
    read("../middleware.ts"),
    read("../app/api/player/route.ts"),
    read("../supabase/migrations/202608050001_admin_users.sql"),
  ]);

  assert.match(auth, /ROBLOX_OAUTH_CLIENT_ID/);
  assert.match(auth, /userIsAuthorized/);
  assert.match(middleware, /rollwatch_session/);
  assert.match(playerRoute, /requireAdminRequest/);
  assert.match(playerRoute, /validatePlayerData/);
  assert.match(playerRoute, /method: "PATCH"/);
  assert.match(migration, /create table if not exists public\.admin_users/);
  assert.match(migration, /enable row level security/);
});

test("legacy player saves get safe datastore defaults", async () => {
  const [playerData, bootstrapCss] = await Promise.all([
    read("../lib/player-data.ts"),
